...
def run_model(I, F, R, M, A, pretrained_CNN, save_filetag, num_sessions, irun=0, percent_changed_pixels=0.1):
	...
	for isession in range(num_sessions):
		...
		imgs_train = A.get_images_to_show_near_edges(I, F, R, M, percent_changed_pixels=percent_changed_pixels)
	...
	np.save('./results/near_edges/corrs_test_' + pretrained_CNN + '_' + str(percent_changed_pixels*100) + 'percent_changed_pixels_run{:d}.npy'.format(irun), np.asarray(corrs_test))


def transform_image_to_near_edges(img_raw, percent_changed_pixels=0.1):
	# I: image class
	...
	# img_2d = ndimage.maximum_filter(img_2d, size=mask_size)
...
if True:
	...
	# img_orig = I.get_sky_rafidi_example()
	...
	for percent_changed_pixels in percents_changed_pixels:
		...
		img = transform_image_to_near_edges(img_orig, percent_changed_pixels=percent_changed_pixels)
	...
	f.savefig('./figs/thumbnails_near_edges.pdf')
