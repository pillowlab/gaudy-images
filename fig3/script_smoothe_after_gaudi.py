...
def run_model(I, F, R, M, A, pretrained_CNN, save_filetag, num_sessions, irun=0, smoothing_sigma=1.0):
	...
	for isession in range(num_sessions):
		...
		imgs_train = A.get_images_to_show_smoothing_after_gaudi(I, F, R, M, smoothing_sigma=smoothing_sigma)
	...
	np.save('./results/smoothe_after_gaudi/corrs_test_' + pretrained_CNN + '_' + str(int(smoothing_sigma*100)) + 'smoothing_sigma_run{:d}.npy'.format(irun), np.asarray(corrs_test))


def transform_image_to_smoothe_after(img_raw, smoothing_sigma=1.0):
	# I: image class
...
# train models with different contrast levels
	...
	smoothing_sigmas = [0., 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2., 2.25, 2.5]
...
if True:
	...
	for smoothing_sigma in smoothing_sigmas:
		...
		img = transform_image_to_smoothe_after(img_orig, smoothing_sigma=smoothing_sigma)
	...
	f.savefig('./figs/thumbnails_smoothing_after_gaudi.pdf')
