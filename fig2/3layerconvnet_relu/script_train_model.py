...
def run_model(I, F, R, M, A, pretrained_CNN, save_filetag, run_id, num_sessions):
...
images_type = sys.argv[3]  # {'random_selection', 'gaudy'}
...
M = class_model.ModelEnsembleClass(num_models=num_models, num_output_vars=100, learning_rate=learning_rate, seed_index=seed_index)   # goes first for GPU allocation
	# give new seed index for each run
...
### gaudy images
if images_type == 'gaudy':
	A = class_gaudy.gaudySelectionClass()
	run_model(I, F, R, M, A, pretrained_CNN, 'gaudy', run_id, num_sessions)
