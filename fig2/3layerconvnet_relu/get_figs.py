	...
	for pretrained_CNN in pretrained_CNNs:
		...
		for image_type in ['random_selection', 'gaudi']:
			...
			for irun in range(num_runs):
				frac_vars_runs[irun] = np.load('./results/frac_vars_test_' + pretrained_CNN + '_' + image_type + '_run{:d}.npy'.format(irun))[:30]
...
# find out mean +/- std dev for performance after 100 sessions (VGG19)
if True:
	pretrained_CNN = 'VGG19'
	image_type = 'random_selection'
	frac_vars_runs = np.zeros((5,))
	for irun in range(5):
		f = np.load('./results/frac_vars_test_' + pretrained_CNN + '_' + image_type + '_run{:d}.npy'.format(irun))
		frac_vars_runs[irun] = np.load('./results/frac_vars_test_' + pretrained_CNN + '_' + image_type + '_run{:d}.npy'.format(irun))[99]

	print('mean = {:f} +/- {:f}'.format(np.mean(frac_vars_runs), np.std(frac_vars_runs)))
