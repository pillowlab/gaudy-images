...
class ModelEnsembleClass: 
	...
	def compute_frac_var(self, features_test, responses_test):
		...
		frac_var = np.mean(np.diagonal(np.corrcoef(responses_hat, responses_test)[:num_neurons,num_neurons:])**2)
		return frac_var
	...
	def save_models(self, filetag='base_model'):
		# stores ensemble's weights for later use

		self.base_model.save(self.save_folder + filetag + '.h5')
		with open(self.save_folder + 'models_weights_' + filetag + '.pkl', 'wb') as fp:
			pickle.dump(self.models_weights, fp)

		with open(self.save_folder + 'models_optimizer_states_' + filetag + '.pkl', 'wb') as fp:
			pickle.dump(self.models_optimizer_states, fp)



	def load_models(self, filetag='base_model'):
		# loads ensemble's weights

		self.base_model = load_model(self.save_folder + filetag + '.h5')
		with open(self.save_folder + 'models_weights_' + filetag + '.pkl', 'rb') as fp:
			self.models_weights = pickle.load(fp)

		with open(self.save_folder + 'models_optimizer_states_' + filetag + '.pkl', 'rb') as fp:
